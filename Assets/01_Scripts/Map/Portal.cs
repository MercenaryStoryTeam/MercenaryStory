...
public class Portal : MonoBehaviour
{
	...
	private void IsOnPortal()
	{
		...
		Collider[] hitColliders =
			Physics.OverlapBox(center, size, gameObject.transform.rotation, layerMask);
		{
			foreach (var hitCollider in hitColliders)
			{
				if (hitCollider.gameObject == StageManager.Instance.hostPlayerFsm.gameObject &&
				    StageManager.Instance.StageClear)
				{
					ServerManager.LeaveAndLoadScene("");
				}
			}
		}
	}
