...
public class MonsterAttackState : State<Monster>
{
	public override void EnterState(Monster monster)
	{
		...
		SoundManager.Instance.PlaySFX(monster.monsterData.attackSound,
			monster.gameObject);
	}
	...
	public override void ExecuteState(Monster monster)
	{
		...
		{
			Vector3 direction =
				(monster.TargetTransform.position - monster.transform.position)
				.normalized;
			Quaternion lookRotation = Quaternion.LookRotation(direction);
			monster.transform.rotation = Quaternion.Slerp(monster.transform.rotation,
				lookRotation, monster.RotationSpeed * Time.deltaTime);
		}
		...
		if (Vector3.Distance(monster.transform.position, monster.patrolPoint) >
		    monster.ReturnRange)
		{
