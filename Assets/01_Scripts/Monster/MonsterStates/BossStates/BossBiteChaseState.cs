...
public class BossBiteChaseState : State<BossMonster>
{
	...
	public override void EnterState(BossMonster boss)
	{
		...
		{
			float exDistance = Vector3.Distance(
				boss.minionList[minionNum].gameObject.transform.position,
				boss.transform.position);
			float distance = Vector3.Distance(
				boss.minionList[i].gameObject.transform.position,
				boss.transform.position);
	...
	public override void ExecuteState(BossMonster boss)
	{
		...
		Collider[] targets = Physics.OverlapSphere(boss.transform.position,
			boss.slashAttackRange, boss.MinionLayer);
		if (targets.Length > 0)
		{
