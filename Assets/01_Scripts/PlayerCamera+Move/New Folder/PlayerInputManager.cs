...
public class PlayerInputManager : MonoBehaviourPun
{
    ...
    public static System.Action OnEscapePressed; // ESC 키 이벤트 추가
    ...
    void Update()
    {
        ...
        {
            OnEscapePressed?.Invoke(); // ESC 키 이벤트 호출
            HandleEscapeKey();
        }
    }
    ...
    private void HandleEscapeKey()
    {
        ...
        // SkillUIManager가 활성화되어 있다면 비활성화
        if (skillUIManager != null && skillUIManager.skillPanel.activeSelf)
        {
            skillUIManager.CloseSkillUpgradeUI();
        }
    }
    ...
    private void HandleDesktopInputs()
    {
        ...
        // ESC 키 관련 코드를 제거 (이미 Update에서 처리)
    }
