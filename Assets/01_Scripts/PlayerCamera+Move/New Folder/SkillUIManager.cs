...
public class SkillUIManager : MonoBehaviour
{
	...
	// 기본 버튼 색상 저장
	private Dictionary<Button, Color> buttonDefaultColors =
	...
	{
		goldText.text =
			FirebaseManager.Instance.CurrentUserData.user_Gold.ToString();
	}
	...
	private IEnumerator FindReferencesRoutine()
	{
		while (true)
		{
			if (skillFsm == null || player == null)
			{
				...
				{
					Debug.LogWarning(
						"[SkillUpgradeUI] Player 인스턴스를 찾지 못했습니다. 1초 후 다시 시도합니다.");
				}
				else
				{
					// 플레이어의 골드 변경 이벤트 구독
				...
				{
					Debug.LogWarning(
						"[SkillUpgradeUI] SkillFsm 참조를 찾지 못했습니다. 1초 후 다시 시도합니다.");
				}
			}
			else
			{
		...
		if (skillButtons != null)
		{
			for (int i = 0; i < skillButtons.Count; i++)
			{
				...
				if (button != null)
				{
					...
					{
						Debug.LogWarning(
							$"[SkillUpgradeUI] 스킬 타입이 버튼 수보다 적습니다. 버튼: {button.name}");
					}
				}
			}
		}
	...
	public void UpgradeSelectedSkill()
	{
		...
		{
			Debug.LogWarning(
				$"[SkillUpgradeUI] {selectedSkill.skillType} 스킬은 이미 최대 레벨({selectedSkill.MaxLevel})에 도달했습니다.");
			return;
		}

		// 다음 레벨 업그레이드 비용 가져오기
		float upgradeCost =
			selectedSkill.UpgradeCosts[selectedSkill.Level - 1]; // 레벨은 1부터 시작
		...
		if (player.SpendGold(upgradeCost))
		{
			...
			{
				Debug.Log(
					$"[SkillUpgradeUI] {selectedSkill.skillType} 스킬을 업그레이드했습니다. 현재 레벨: {selectedSkill.Level}");
			...
			{
				Debug.LogWarning(
					$"[SkillUpgradeUI] {selectedSkill.skillType} 스킬 업그레이드에 실패했습니다.");
			}
		}
		else
		{
