...
public class Skill
{
	...
	[Header("최대 스킬 레벨")] public int MaxLevel = 4;

	[Header("스킬 레벨별 업그레이드 비용")] public List<float> UpgradeCosts;

	[Header("기본 쿨타임 (초 단위)")] public float BaseCooldown = 2f;

	[Header("레벨당 쿨타임 감소 비율 (백분율)")] public int CooldownReductionPerLevel = 5;

	[Header("Rush 스킬만 해당 (이동 속도 배수 처리)")] public float SpeedBoost = 0f;

	[Header("Rush 스킬만 해당 (지속 시간)")] public float Duration = 0.5f;

	[Header("스킬 설명")] [TextArea] public string Description;

	[Header("스킬 이미지")] public Sprite Icon;

	[Header("레벨에 따른 이펙트 및 스폰 포인트 리스트")] public List<SkillEffect> SkillEffects;
...
public class SkillFsm : MonoBehaviour
{
	...
	[Header("Player 스크립트")] public Player player;

	[Header("디버그 설정 (출력 유/무)")] public bool enableDebugLogs = true;
